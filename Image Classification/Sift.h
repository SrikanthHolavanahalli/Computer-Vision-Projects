...
class Sift 
{
  ...
  static vector<SiftDescriptor> compute_sift(const CImg<float> &luma)
    {
      ...
      // ---------------------------------------------------------------    
      int         O      = octaves ;    
      int constexpr   S      = levels ;
      int constexpr   omin   = first ;
      float constexpr sigman = .5 ;
      float constexpr sigma0 = 1.6 * powf(2.0f, 1.0f / S) ;
 ...
 protected:
  static constexpr int octaves = 7, levels = 3, first = -1; 
